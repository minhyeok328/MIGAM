...
React·TypeScript·Vite 기반의 미감 브랜드 홈과 전시·기관·작품 탐색 화면입니다. `/`는 자체 생성한 가상 미술관 필름, 세 이미지 섹션과 탐색 CTA를 제공하며 API를 호출하지 않습니다. `/discover`는 전시·기관 검색과 조건 추천을 소유하며, 검색은 카탈로그 카드, 추천은 에디토리얼 카드로 구분합니다. Tailwind CSS, 직접 사용하는 Radix Dialog/Tabs, Lucide React를 사용합니다. 상세·취향·관심·비교·초기화와 전시장 지도를 연결했고, 실제 작품은 출처 심사가 끝날 때까지 준비 상태로 표시합니다.
...
2026-09-17 기존 무료 쿼터 앱 `1528032`를 미감으로 전환했다. 일반 개발 실행(`npm run dev`, `http://127.0.0.1:5173`)은 Git에서 제외된 `.env.development.local`의 `VITE_KAKAO_JAVASCRIPT_KEY`에 이 앱의 공개 JavaScript 키를 설정하면 내장 지도를 사용할 수 있다. 최신 사용자 결정에 따라 전시 상세 아래에서 지도를 자동으로 불러오고, 지도 안 오른쪽 위에 내 위치·확대·축소·전시장으로 돌아가기 아이콘 버튼을 둔다. 검토된 장소명과 공식 주소가 일치하는 장소를 바로 표시하고, 이름 검색에 결과가 없으면 검토된 공식 주소 위치를 표시한다. 장소명·주소와 주소 위치의 한계는 스크린리더용 설명으로 유지한다. 별도 제목·주소·성공 안내·외부 지도 링크는 제거한다. 내 위치 버튼을 직접 눌렀을 때만 브라우저 위치 권한을 요청하며 좌표는 현재 지도 메모리에서만 사용한다. 위치 상태는 지도 안에서 닫을 수 있는 메시지로 알리고, 전시장으로 돌아가기는 최초 전시장 위치와 확대 수준으로 복귀한다. 미검토 장소는 후보를 선택해야 하며 로딩·실패·0건에는 짧은 안내와 가능한 재시도만 제공한다. 키를 변경하면 Vite를 재시작한다. 개발 전용 파일이므로 테스트·홈 미리보기·운영 빌드에는 자동 적용되지 않는다. 키 없는 5180 `local-data` 실행과 5181 데모는 환경 파일을 읽지 않으므로 지도 키 주입의 영향을 받지 않는다.
