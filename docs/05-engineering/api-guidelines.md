...
2026-09-17 사용자 결정에 따라 기존 무료 쿼터 앱을 미감으로 전환하고 공개 JavaScript 키를 주입한 일반 개발 환경에서 내장 MapProvider를 사용할 수 있다. 같은 날 사용자가 요청한 지도 동작은 아래와 같다.

- **실행 시점:** 지도 키가 설정된 일반 전시 상세에 진입하면 SDK와 장소 검색을 자동 실행한다. 정상 표시 영역은 지도와 그 안의 조작 버튼으로 구성한다.
- **장소 검색:** 기관·지역·venue가 검토 범위와 일치하면 공식 장소명과 기관명을, 그 외에는 venue와 기관명을 지역과 함께 최대 2회 검색한다. 적합한 후보가 없을 때만 다음 검색을 진행하고 검토된 공식 주소가 있으면 EXACT 주소 검색 1회를 추가한다. API 오류에는 추가 호출하지 않는다.
- **검색 결과 표시:** 검토 대상은 이름·도로명 주소가 일치하는 후보를 자동 표시한다. 같은 주소의 적합한 후보가 여러 개면 검토된 대표 장소명을 우선한다. 미검토 대상은 지역을 대조한 후보를 제공하고 사용자 선택 후 표시한다. 검토된 주소 검색 결과는 전시실·입구가 아닌 `공식 주소 위치`로 구분해 자동 표시한다.
- **화면과 조작:** 장소명·주소와 주소 위치의 한계는 스크린리더용 설명으로 유지한다. 별도 제목·주소·성공 안내·외부 지도 링크는 표시하지 않는다. 지도 안에는 접근 가능한 이름과 키보드 포커스를 갖춘 내 위치·확대·축소·전시장으로 돌아가기 아이콘 버튼을 제공한다. 확대·축소와 최초 전시장 위치·확대 수준 복귀는 MapProvider 뒤의 지도 인스턴스에서 처리한다.
- **위치 권한과 응답:** 내 위치 버튼을 직접 눌렀을 때만 브라우저 위치 권한을 요청한다. 상태 안내는 지도 안에서 닫을 수 있게 제공하고 상세 전환·지도 해제 뒤의 위치 응답은 무시한다.
- **실패와 키 없는 실행:** 로딩·실패·0건·미검토 후보 선택은 짧은 안내를 제공하고 가능한 상태에서만 재시도한다. 키가 없으면 SDK 없이 비가용 안내를 제공한다. 공식 안내는 상단 관람 정보의 기존 전시 링크로 연결한다.

정본 위치로 자동 확정하거나 사용자 원문 검색어·정확 좌표를 전달·저장하는 내부 API를 만들지 않는다. 홈의 API-free 경계, 키 없는 5180 `local-data` 실행과 가상 기관을 실제 지도에 표시하지 않는 데모 경계는 유지한다. 세부 범위와 검토 출처는 [TP-008](../07-execution/task-packets/TP-008-p0-product-completion.md)과 [지도 검색 검증](../06-quality/map-search-verification.md)을 따른다.
