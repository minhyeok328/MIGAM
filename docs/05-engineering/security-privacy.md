...
P0의 모든 기능은 일반 사용자 계정 없이 사용할 수 있어야 한다. 일반 사용자 계정·로그인·서버 익명 프로필·장기 취향 프로필을 만들거나 우회적으로 재구성하지 않는다. 외부 행동 분석 도구를 사용하지 않는다. 보안 설계는 데이터 최소화와 운영자 권한 분리를 기본으로 한다. 출처와 권리는 확인한 내용대로 표시한다.
...
- 2026-09-17 사용자 결정에 따라 키가 설정된 일반 전시 상세는 Kakao SDK와 장소 검색을 자동 실행한다. 외부 검색 입력은 기관·지역·실제 전시장 또는 검토된 공식 주소에 한정하고 사용자 원문 검색어·취향·관심 기록을 결합하지 않는다. 지도 내부 조작 버튼을 추가하는 후속 요청에 따라 내 위치 버튼의 명시적 클릭 후에만 브라우저 위치 권한과 현재 위치를 한 번 요청한다. 상세 진입·지도 자동 표시·확대·축소·전시장으로 돌아가기만으로 위치 권한을 요청하지 않으며 지속 위치 추적을 하지 않는다. 사용자 위치와 장소 좌표는 현재 지도 표시의 메모리에서만 사용하고 서버·로그·브라우저 저장소에 보존하지 않는다. 상세 전환·지도 해제 뒤 도착한 위치 응답은 무시한다. 홈·키 없는 실행·데모는 외부 지도를 자동 호출하지 않는다. 상세 동작과 장소 대조 기준은 [TP-008](../07-execution/task-packets/TP-008-p0-product-completion.md)을 따른다.
