...
사용자 승인에 따라 미사용 빵찾깅 앱 `1528032`를 미감으로 전환하고 무료 쿼터를 유지한 기존 JavaScript 키를 일반 개발 환경에 연결한다. 같은 날 사용자는 전시 상세의 지도 자동 표시와 지도 내부 조작 버튼을 요청했다.

- 키가 설정된 일반 전시 상세에서는 MapProvider를 자동 실행하고 검토된 장소명·주소와 일치하는 장소 또는 공식 주소 위치를 바로 표시한다. 미검토 장소는 후보를 직접 선택한다.
- 지도 주변의 제목·주소·성공 안내·외부 지도 링크는 제거한다. 지도 안에는 내 위치·확대·축소·전시장으로 돌아가기 아이콘 버튼을 제공한다.
- 사용자 위치 권한은 내 위치 버튼을 직접 눌렀을 때만 요청한다. 위치는 현재 지도 메모리에서만 사용하고 지도 안의 닫을 수 있는 메시지로 상태를 알린다. 상세 전환 뒤의 응답은 무시한다.
- 로딩·실패·0건에는 짧은 안내와 가능한 재시도만 제공한다. 키 없는 실행은 SDK 없이 비가용 안내를 표시하며 공식 안내는 상단 관람 정보의 기존 전시 링크를 이용한다.

5181 데모의 외부 지도 금지와 공개 배포 범위는 유지한다. 적용 범위와 확인 결과는 [TP-008](task-packets/TP-008-p0-product-completion.md)의 브라우저 상태와 경로 계약을 따른다. 아래 날짜별 결과의 내장 지도 비활성 상태는 당시의 기록이다.

## 2026-09-07 실제 전시 내용 보강

DEC-119와 [TP-011](task-packets/TP-011-exhibition-content.md)은 링크만 반복하던 상세에 실제 소개·볼거리·관람 안내를 추가한다. 현재 검색되는 17건 모두 소개와 볼거리를, 14건은 요금·운영시간 문구를 확보했다. 기존 확정 방문 조건·수집 이력은 유지한다. 입력 자료와 실제 DB 보존·브라우저 검증 결과는 [TP-011 검증 기록](../06-quality/tp-011-verification.md)을 따른다.

## 2026-09-07 로컬 프로젝트 마무리

사용자의 DEC-118은 당시 로컬 작업의 완료 조건에서 공개 배포를 제외한다. TP-010에서 실제 DB 읽기 전용 단일 origin 실행, 별도 파일 복구, 격리 브라우저 회귀와 CI 설정을 추가했다. 로컬 완료는 승인한 초기 화면·실행·복구·검사 범위를 뜻하며 포괄 P0 전체의 구현 완료와 구분한다. 호스팅·도메인·Docker·운영자 생성·정기 수집 자동화는 필수가 아니다. 실행한 검사와 남은 환경 제약은 [TP-010 검증 기록](../06-quality/tp-010-verification.md)을 따른다. 아래 이전 날짜별 배포 우선순위는 이 결정으로 대체된다.

## 2026-09-07 초기 출시 조정

TP-009에서 전시 둘러보기를 유지하고 기본 날짜를 전시 기간으로 분리한다. 공식 안내 연결·핵심 흐름 회귀·호환성·공개 운영 준비를 우선하며, 선택 관람 정보는 현재·예정 전시부터 점진적으로 보강한다. 모든 가격·운영일·예약 정보 확보를 출시 선행 조건으로 두지 않는다. 실제 작품 Source와 포괄 P0 잔여 요구는 별도이며 전체 완료로 간주하지 않는다. 최신 실행 결과는 [TP-009 검증 기록](../06-quality/tp-009-verification.md)을 따른다.

## 현재 판단

P0의 제품 방향·기술 구성·개인정보 금지 조건을 기준으로 작업을 나눌 수 있다. 이 문서는 구현 준비도를 판단하는 기준이며 전체 구현 계획이나 일정표는 다루지 않는다. 실제 구현은 각 작업 패킷이 관련 제품·엔지니어링 기준과 아래 작업 시작 조건을 충족하는지 확인한 뒤에만 시작한다.
...
여섯 패킷은 Source·기관 운영 상태, 공용 게이트, 기관별 결과·health, 자격 실행의 ChangeHistory·QualificationRun·PromotionEvidence·`PROVISIONAL → ACTIVE` 자동 승격, 선택 정보·권리 정본, 전시·기관 내부 검색과 근거 있는 전시 추천, API 없는 브랜드 홈 `/`와 검색·추천 `/discover`, 키 없는 격리 데모까지 승인했다.

후보 심사 자동화·복구 승인·Admin·작품·작가·운영시간·지도·취향 테스트·상세·비교·관심 저장과 추천 특성의 외부 Source 자동 백필은 이 여섯 패킷 당시의 제외 범위이며, 현재 범위는 다음 TP-007·TP-008을 함께 읽는다.
