...
추천·검색·비교·방문 판단의 주 객체다. 한 전시는 공식적으로 확인된 개최 단위를 나타내며 최소한 정본 식별, 제목, 기간, 개최 장소 또는 기관, 공식 근거를 가져야 한다. 다음 상태축은 서로 분리한다.

- 생명주기: 예정, 진행 중, 종료, 취소, 미확인
- 최신성: 최신, 오래됨, 검증 불가
- 노출 적격성: 검증, 일부 확인, 발견 전용, 제외
...
| PriceOption | 대상, 금액 또는 범위, 무료 여부, 기본권·할인·프로그램 구분 |
| ReservationInfo | 예약 유형, 공식 링크, 안내 문구; 잔여석·매진 상태는 포함하지 않음 |
| VisitDuration | 공식값, 객관 근거 기반 참고 범위, 미확인을 구분 |
| AccessibilityFact | 휠체어 접근, 이동, 자막, 수어, 오디오 설명, 연령 조건의 확인 상태 |
| SensoryNotice | 큰·갑작스러운 소리, 섬광, 어두움, 좁거나 밀폐된 공간의 확인 상태 |
...
승인된 공식 API, 데이터 파일, 웹페이지 또는 공지를 나타낸다. 책임 주체, 접근 방식, 이용조건·라이선스, 허용 필드, 운영 상태와 정책 검토를 연결한다.
...
| IngestionRun | 출처별 실행 범위, 성공·실패·건너뜀과 처리량을 추적 |
| ChangeHistory | 정본값·상태·권리 판정의 변경 전후와 근거를 추적 |
...
추천에 사용할 승인된 콘텐츠 특성의 버전 있는 스냅샷이다. 매체·주제·분위기·감상 방식·방문 조건과 근거 상태를 담을 수 있다. 추천 알고리즘과 가중치는 이 모델이 아니라 추천 명세가 정한다.
...
| OD-003 | 실제 출처 allowlist에 따라 Source와 출처별 원본 식별자 매핑 범위를 확정한다. |
