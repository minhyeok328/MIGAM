...
2026-09-06 작품 Source 후보 OA-15321(`SemaPsgudInfoKorInfo`) 공개 sample 5건과 공식 데이터셋을 확인했다. 6,406건이 있으나 공공누리 제4유형·제3자 저작권 있음, API의 안정 작품 ID·공식 상세 URL·이미지별 권리 필드 부재 때문에 `HOLD`다. 수집연도는 고유 ID로 쓰지 않고 제목·이미지 파일명으로 임의 연결하지 않는다. 최신 공식 페이지는 https://data.seoul.go.kr/dataList/OA-15321/S/1/datasetView.do 이며 옛 검색 캐시의 권리 표시를 재사용하지 않는다. 작품 경로와 키 없는 데모 구현은 진행하되 이 Source의 실제 레코드·이미지를 정상 서비스에 적재하지 않는다.
...
2026-09-21 운영 배포 검증에서 기존 수원 전시 `394181`·`368076`의 공식 `detail2` 응답을 확인했다. 장소는 `수원시립미술관`, 주소는 `경기도 수원시 팔달구 정조로 833 수원시립미술관`으로 반환됐다. 기존 승인 주소 `경기도 수원시 팔달구 정조로 833`에 같은 기관명이 붙은 표기이므로 `sources.yaml`에 두 문자열을 정확 일치 허용값으로 기록한다. 기관·물리적 장소·기간·공식 URL 기준은 유지하며 주소 접두사나 임의 건물명으로 확장하지 않는다. 문화정보 기관의 사실 필터는 단일 문자열 또는 검토된 정확 일치 문자열 목록을 사용할 수 있다.

같은 상세 응답의 URL에는 XML 해석 후에도 HTML 엔티티 `&amp;`가 남아 있었다. URL 사실값은 HTML 엔티티를 한 번 해석해 실제 쿼리 구분자로 사용하고, 허용 필드의 원본 값은 `raw`에 그대로 보존한다. 호스트·경로·URL 품질 검증은 해석된 URL에 계속 적용한다.
...
### 2026-09-06 TP-008 추가 심사

사용자의 P0 E2E 위임에 따라 기존 승인 Source `OA-15323`의 물리 기관 4곳을 추가 심사했다. 새 네트워크 수집 없이 2026-09-06 취득한 공식 CSV(SHA-256 `25862cf046a97ce8319493356155b3612a5ba788d6ef4419a21011ad660e6906`)를 재사용했다. 전시 상세 경로의 실제 전시만 표본으로 삼고 `/series/` 교육·행사 행은 전시 표본에서 제외했다. 각 기관 최근 시작일 순 5건의 ID·제목·기간·장소·지역·공식 상세 URL을 [추가 fixture](../../fixtures/source-expansion-2026-09-06.json)에 보존하고 동일 품질 파이프라인으로 전 항목을 검증했다.

| 물리 기관 | 최근 전시 표본 ID | CORE_PASS | 지역 근거 | 판정 |
| --- | --- | --- | --- | --- |
| 서소문본관 | 1559068, 1529410, 1523485, 1509709, 1478064 | 5/5 | [공식 방문 안내](https://sema.seoul.go.kr/kr/visit/seosomun)의 중구 덕수궁길 61 | PROVISIONAL |
| 북서울미술관 | 1563285, 1549929, 1538201, 1521429, 1512457 | 5/5 | [공식 전시·기관 안내](https://sema.seoul.go.kr/kr/whatson/exhibition/detail?exNo=1563285)의 노원구 동일로 1238 | PROVISIONAL |
| 남서울미술관 | 1556711, 1544638, 1446062, 1348834, 1299359 | 5/5 | [공식 방문 안내](https://sema.seoul.go.kr/kr/visit/namseoul)의 관악구 남부순환로 2076 | PROVISIONAL |
| 미술아카이브 | 1576838, 1550876, 1513028, 1432256, 1375994 | 5/5 | [공식 방문 안내](https://sema.seoul.go.kr/kr/visit/art_archive)의 종로구 평창문화로 101 | PROVISIONAL |

20건 모두 핵심 필드 구조 누락·날짜 역전·다른 장소·잘못된 상세 ID 0건이다. 이미 검증된 공공누리 1유형 HTTPS CSV의 허용 필드·호출 제한을 유지한다. 기관 HTML은 지역·표본 교차검증에만 사용한다. 가격·운영일의 CSV 공란은 UNKNOWN으로 유지한다. `기타` 장소는 물리 기관을 특정할 수 없어 제외하고, 백남준을 기억하는 집은 CSV의 고유 전시가 4건뿐이므로 5건 표본 요건을 충족하지 못해 이번 확대에서 제외한다. 14일·3회 성공을 갖추지 않았으므로 ACTIVE 승격은 하지 않는다. 아래 5곳 표는 2026-08-30 초기 승인 이력이다.
