...
2026-09-07 사용자 승인 DEC-119: 상세 페이지에 실제 전시 소개·볼거리·확인한 관람 안내를 채운다. 미확인 값마다 공식 링크를 반복한 TP-009의 상세 표현을 대체하고 하나의 공식 안내로 묶는다. 기존 allowlist 공식 상세의 짧은 사실 요약을 출처·확인일과 함께 추가하며 장문 원문·이미지 수집이나 방문 조건 자동 판정 확대는 포함하지 않는다. 현재·예정 전시부터 보강하고 공개 배포 제외 결정은 유지한다. 소유 문서는 Data Model·Data Source Policy·Normalization Rules·Screen Spec·TP-011이다.

2026-09-07 사용자 승인 DEC-118: 공개 배포 없이 로컬 실행 프로젝트로 마무리한다. TP-010의 단일 origin 실제 데이터 실행·별도 파일 복원·격리 브라우저 회귀와 로컬 절차를 완료 기준에 포함한다. CI 설정은 코드 회귀 검사이며 서비스 배포가 아니다. TP-009 검증 기록의 호스팅·공개 운영 준비와 Docker 검증을 현재 완료 선행 조건에서 제외한다. 실제 작품 Source·관람 정보 확대·포괄 P0 잔여 요구는 후속 선택 범위로 유지한다. 소유 문서는 Project Brief·Local Delivery·Test Plan·TP-010이다.

2026-09-07 사용자 승인 DEC-117: 초기 출시는 전시 둘러보기·취향 추천·상세·관심·비교와 공식 안내 연결을 중심으로 한다. 기본 날짜는 전시 기간 검색이며 예산·예약·예상 관람시간 필터는 화면에서 제외한다. 미확인 값·안전조건을 충족으로 만들지 않고 기존 `visit_dates`의 엄격한 의미는 유지한다. 모든 선택 관람 정보 확보를 출시 선행 조건으로 두지 않는다. 이 초기 출시 범위는 TP-007·TP-008의 기본 화면에서 요구하던 방문일·예산·예약·관람시간 자동 판정 범위를 대체하며, 내부 API와 후속 P0 목표를 폐기하지 않는다. 소유 문서는 Project Brief·Domain Rules·Recommendation Spec·Screen Spec·TP-009다.
